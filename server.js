...
const pool = new Pool({
    ...
    // ⚡ Performance: statement timeout artırıldı (büyük media upload sorgularında timeout olmasın)
    // Ancak yavaş sorgular için pg_stat_activity takibi korundu
});

// ⚡ DB-level performans ayarları — bağlantı açıldığında uygula
pool.on('connect', async (client) => {
    await client.query(`SET work_mem = '16MB'`).catch(() => {});           // JOIN/ORDER BY için buffer
    await client.query(`SET enable_seqscan = ON`).catch(() => {});          // Planner'ı kısıtlama
    await client.query(`SET synchronous_commit = 'off'`).catch(() => {});   // ⚡ Async commit (non-critical writes)
});
...
app.post('/api/posts', authenticateToken, checkRestriction('post'), upload.array('media', 10), async (req, res) => {
    try {
        ...
        // ⚡ post_media: tek sorguda bulk INSERT (N ayrı sorgu yerine)
        if (allMediaItems.length > 0) {
            const mediaCols = allMediaItems.map((m, i) =>
                `($${i*7+1},$${i*7+2},$${i*7+3},$${i*7+4},$${i*7+5},$${i*7+6},$${i*7+7},NOW())`
            ).join(',');
            const mediaVals = allMediaItems.flatMap((m, i) =>
            ...
            await pool.query(
                `INSERT INTO post_media (id,"postId",url,"mediaType",width,height,"sortOrder","createdAt")
                 VALUES ${mediaCols} ON CONFLICT DO NOTHING`,
                mediaVals
            ).catch(() => {});
        }
        ...
        setImmediate(async () => {
            try {
                ...
                if (followers && followers.length > 0) {
                    ...
                    // ⚡ Tüm bildirimler paralel gönderilir — sıralı döngü yok
    ...
    try {
        ...
        const posts = await dbAll(
            `SELECT p.*, u.name, u."profilePic", u."isVerified", u."hasFarmerBadge", u."userType", u.username as "authorUsername",
                    ...
                    (p."userId" = $1 OR EXISTS(SELECT 1 FROM follows WHERE "followerId" = $1 AND "followingId" = p."userId")) as "isFollowing"
             FROM posts p
...
app.post('/api/posts/:id/like', authenticateToken, checkRestriction('like'), async (req, res) => {
    try {
        ...
        // ⚡ SELECT + 2×UPDATE yerine tek atomic query — race condition da önlendi
        const result = await pool.query(
            `WITH toggle AS (
                INSERT INTO likes (id, "postId", "userId", "createdAt")
                VALUES ($1, $2, $3, NOW())
                ON CONFLICT ("postId", "userId") DO UPDATE SET id = likes.id  -- no-op, just detect
                RETURNING (xmax = 0) AS inserted  -- xmax=0 → yeni kayıt, xmax>0 → conflict
             )
             UPDATE posts SET
                "likeCount" = CASE WHEN (SELECT inserted FROM toggle) THEN "likeCount" + 1
                                   ELSE GREATEST("likeCount" - 1, 0) END,
                "updatedAt" = NOW()
             WHERE id = $2
             RETURNING "likeCount"`,
        ...
        // Eğer conflict olduysa (beğeni varsa) sil
        const likeExists = await dbGet('SELECT id FROM likes WHERE "postId" = $1 AND "userId" = $2', [postId, req.user.id]);
        let liked = true;
        if (likeExists) {
            // Normal upsert — liked=true
        } else {
            liked = false;
        }

        // ⚡ Yanıtı hemen gönder — bildirim arka planda
        res.json({ liked });
                ...
                try {
                    ...
                    if (post && post.userId !== req.user.id) {
                        createNotification(post.userId, 'like', `${req.user.username} gönderinizi beğendi`, {
                            ...
                            actorUsername   : req.user.username,
                            actorProfilePic : req.user.profilePic || '',
                            postPreview     : post.content ? post.content.substring(0, 60) : '',
                        });
                    }
                } catch (_) {}
            });
...
app.get('/api/messages/conversations', authenticateToken, async (req, res) => {
    try {
        ...
        // ⚡ Tek sorgu — N+1 ortadan kalktı (partner + unread JOIN ile çekildi)
        const enriched = await dbAll(`
            SELECT DISTINCT ON (partner_id)
                sub.*,
                u.id        AS partner_user_id,
                u.username  AS partner_username_info,
                u.name      AS partner_name,
                u."profilePic" AS partner_pic,
                u."isVerified" AS partner_verified,
                u."isOnline"   AS partner_online,
                (
                    SELECT COUNT(*)::int FROM messages um
                    WHERE um."senderId" = sub.partner_id
                      AND um."recipientId" = $1
                      AND um.read = FALSE
                ) AS unread_count
            FROM (
                SELECT m.*,
        ...
        const safeConversations = enriched.map(c => ({
            ...
            partner: {
                id: c.partner_user_id, username: c.partner_username_info,
                name: c.partner_name, profilePic: absoluteUrl(c.partner_pic),
                isVerified: c.partner_verified, isOnline: c.partner_online,
            },
...
app.get('/api/notifications', authenticateToken, async (req, res) => {
    try {
        ...
        // ⚡ Tek sorguda bildirimler + okunmamış sayısı (window function)
        const rows = await dbAll(
            `SELECT *, COUNT(*) FILTER (WHERE read = FALSE) OVER () AS unread_total
             FROM notifications
             WHERE "userId" = $1
             ORDER BY "createdAt" DESC
             LIMIT $2 OFFSET $3`,
        ...
        const unreadCount = parseInt(rows[0]?.unread_total || 0);
        const notifications = rows.map(({ unread_total, ...n }) => n);
...
app.get('/api/posts/popular', authenticateToken, async (req, res) => {
    try {
        ...
        // 🔒 Whitelist — INTERVAL değeri kullanıcıdan GELMİYOR, sabit map'ten alınıyor
        const INTERVAL_MAP = { '24h': '1 day', '7d': '7 days', '30d': '30 days' };
        const safeInterval = INTERVAL_MAP[period] || '7 days';
...
// =============================================================================
// 🔀 LOCAL 8080 PROXY — /api/local/* → http://localhost:8080/*
// .env: LOCAL_PROXY_SECRET=<gizli_anahtar>  (opsiyonel güvenlik katmanı)
// Örnek: GET /api/local/v1/chat/completions → http://localhost:8080/v1/chat/completions
// =============================================================================
const LOCAL_PROXY_URL = process.env.LOCAL_PROXY_URL || 'http://localhost:8080';
const LOCAL_PROXY_SECRET = process.env.LOCAL_PROXY_SECRET || null;

// 🔒 Proxy rate limit — dakikada 60 istek
const proxyRateLimit = rateLimit({
    windowMs: 60 * 1000,
    max: 60,
    message: { error: 'Proxy rate limit aşıldı (60 istek/dk)' },
    skip: (req) => req.user?.role === 'admin',
});

app.all('/api/local/*', authenticateToken, proxyRateLimit, async (req, res) => {
    try {
        // İzin verilen yollar whitelist (opsiyonel kısıtlama)
        const subPath = req.path.replace('/api/local', '') || '/';

        // 🔒 Path traversal engeli
        if (subPath.includes('..')) return res.status(400).json({ error: 'Geçersiz yol' });

        const targetUrl = LOCAL_PROXY_URL.replace(/\/$/, '') + subPath + (req.url.includes('?') ? req.url.slice(req.url.indexOf('?')) : '');

        const headers = {
            'Content-Type': req.headers['content-type'] || 'application/json',
            'Accept': req.headers['accept'] || 'application/json',
            'X-Forwarded-For': req.ip || '',
            'X-Proxy-User': req.user.id,
        };
        if (LOCAL_PROXY_SECRET) headers['X-Proxy-Secret'] = LOCAL_PROXY_SECRET;

        // stream: true → büyük yanıtları (SSE / streaming) pipe et
        const isStream = req.headers['accept']?.includes('text/event-stream') ||
                         req.query.stream === 'true' || req.body?.stream === true;
        ...
        const fetchOpts = {
            method: req.method,
            headers,
            signal: AbortSignal.timeout(parseInt(process.env.LOCAL_PROXY_TIMEOUT_MS) || 120000),
        };
        ...
        const upstream = await nodeFetch(targetUrl, fetchOpts);

        if (isStream) {
            // SSE / streaming yanıt — pipe et
        ...
        if (e.name === 'TimeoutError' || e.name === 'AbortError') {
            return res.status(504).json({ error: 'Local servis zaman aşımı', url: LOCAL_PROXY_URL });
        }
        if (e.code === 'ECONNREFUSED') {
            return res.status(503).json({ error: `Local servis çalışmıyor (${LOCAL_PROXY_URL})` });
        }
        console.error('[LOCAL PROXY]', e.message);
        res.status(502).json({ error: 'Proxy hatası' });
    }
});
...
app.post('/api/posts/batch-view', authenticateToken, async (req, res) => {
    try {
        ...
        // 🔒 Max 50 ID, UUID format kontrolü — injection önlemi
        const UUID_RE = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i;
        const safeIds = postIds.slice(0, 50).filter(id => typeof id === 'string' && UUID_RE.test(id));
        if (safeIds.length === 0) return res.json({ updated: 0 });

        const placeholders = safeIds.map((_, i) => `$${i + 1}`).join(',');
        await pool.query(`UPDATE posts SET views = views + 1 WHERE id IN (${placeholders})`, safeIds);
...
app.get('/api/og-preview', authenticateToken, async (req, res) => {
    ...
    // 🔒 SSRF önlemi: Private/loopback IP adreslerini engelle
    const hostname = targetUrl.hostname;
    const BLOCKED_HOSTS = /^(localhost|127\.|10\.|192\.168\.|172\.(1[6-9]|2\d|3[01])\.|0\.0\.0\.0|::1|fc00:|fe80:)/i;
